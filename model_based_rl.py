...
def build_dynamics_graph(scope,
    ...
    _prediction_losses = [
        tf.reduce_mean(
            tf.reduce_sum(
                get_ith_tensor(dvds_weight_full, i, n_states) *
                tf.square(
    ...
    if not use_value:
        ...
        dynamics_losses = [
            ...
            for i in range(len(_value_difference_losses))
        ]
        dynamics_loss = tf.add(value_difference_loss, regularizer_loss,
                               name='total_dynamics_loss')
...
def train_models(env,
    ...
    if dynamics_opt_params.dvds_weighting:
        assert dynamics_opt_params.use_value == False
    ...
    value_f = tf.placeholder(tf.float32, shape=(1, n_states * 2 + 4), name='value_f')
    ...
    # TODO: add forward performing code here
    #################
    ## Performing ###
    #################
    if variant["perform"] == True:
        ...
        for i in range(10):
            collect_data(env,
                         ...
                         sess,
                         None,  # for param_noise exploration
    ...
    while True:
        ...
        dynamics_learning_logs = optimize_models(dynamics_data,
                                                 ...
                                                 lr_ph,
                                                 reinitialize,
                                                 dynamics_opt_params.dvds_weighting
                                                 )
    ...
    for scope in dynamics_data.keys():
        ...
        min_sum_validation_loss, min_validation_losses = sess.run(
            ...
            feed_dict={
                ...
                dvds_weight_full: dvds_batch_val,
        ...
        for j in range(1, max_iters + 1):
            ...
            _, training_loss = sess.run([dynamics_opt_op[scope], dynamics_loss[scope]],
                                        feed_dict={
                                            ...
                                            dvds_weight_full: np.reshape(dvds_batch, (batch_size, -1)),
                                            value_f_ph: coeff_batch
                                        })
